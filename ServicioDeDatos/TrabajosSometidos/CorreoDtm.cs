...
namespace ServicioDeDatos.TrabajosSometidos
{
    ...
    public static class TablaDeCorreos
    {
        public static void Definir(ModelBuilder mb)
        {
            ...
            mb.Entity<CorreoDtm>().Property(p => p.Elementos).HasColumnName("ELEMENTOS").IsRequired(true).HasColumnType("VARCHAR(2000)");
            mb.Entity<CorreoDtm>().Property(p => p.Archivos).HasColumnName("ARCHIVOS").IsRequired(true).HasColumnType("VARCHAR(MAX)");
